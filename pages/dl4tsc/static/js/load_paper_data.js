...
$(document).ready(function() {
	$.getJSON("static/jsons/paper_data.json", function(data){
		...
		$("#img-overview2").attr("src", data.image_overview2);
		$("#caption-overview2").append(data.caption_overview2);
		...
		if (data.hasOwnProperty('external_htmls')) {
			for (var i=0; i<data.external_htmls.length; i++){
				$.get(data.external_htmls[i], function (html_data) {
					$('#specific-parts').append(html_data);
				});
			}
		}
	});
